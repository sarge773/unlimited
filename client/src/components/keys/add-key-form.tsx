...
=======
          <Select value={platform} onValueChange={(v) => setPlatform(v as Platform)}>
            ...
            <SelectContent>
              {PLATFORMS.map(p => (
                <SelectItem key={p.value} value={p.value} className="whitespace-nowrap">{p.label}</SelectItem>
              ))}
            </SelectContent>
          </Select>
>>>>>>> 48fc9f5 (fix(ui): widen provider select to prevent description truncation (#945))

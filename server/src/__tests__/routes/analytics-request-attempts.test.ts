...
function insertAttempt(requestId: number, ordinal: number, platform: string, modelId: string, keyOrdinal: number, outcome: string, startOffsetMs: number, durationMs: number) {
  getDb().prepare(`
    INSERT INTO request_attempts (request_id, ordinal, platform, model_id, key_ordinal, outcome, start_offset_ms, duration_ms)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(requestId, ordinal, platform, modelId, keyOrdinal, outcome, startOffsetMs, durationMs);
}
...
describe('per-attempt traces in the analytics requests API', () => {
  ...
  it('returns the ordered attempts ladder in the per-request detail payload', async () => {
    const id = insertRequest('success', recentSql(12));
    insertAttempt(id, 0, 'groq', 'llama-x', 1, 'rate_limited', 0, 210);
    insertAttempt(id, 1, 'google', 'gemini-y', 2, 'timeout', 215, 10000);
    ...
    expect(body.attempts).toEqual([
      { ordinal: 0, platform: 'groq', modelId: 'llama-x', keyOrdinal: 1, outcome: 'rate_limited', startOffsetMs: 0, durationMs: 210 },
      { ordinal: 1, platform: 'google', modelId: 'gemini-y', keyOrdinal: 2, outcome: 'timeout', startOffsetMs: 215, durationMs: 10000 },
      { ordinal: 2, platform: 'cerebras', modelId: 'qwen-z', keyOrdinal: 3, outcome: 'ok', startOffsetMs: 10220, durationMs: 900 },
    ]);
  });
